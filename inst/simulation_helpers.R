...
compare_model_fits <- function(d, with_smdm=FALSE) {
  ...
  result_list = list(
    ...
    hat_weighted_rank = hat_weighted_rank, 
    lme = lme
  )
  
  if (with_smdm) {
    koller <- rlmer(f, data = dat)
    result_list[[5]] <- koller
    names(result_list)[5] <- "koller"
  }
...
get_coefficients_from_model <- function(fitted_object) {
  ...
  hat_weighted_rank = with(fitted_object,
                       list(beta = hat_weighted_rank$beta,
                            variances = c(hat_weighted_rank$sigma, hat_weighted_rank$theta))
  )
  ...
  return(
    data.frame(
      ...
      hat_weighted_rank = unlist(hat_weighted_rank), 
...
get_coefficients_from_simulation <- function(fitted_object) {
  ...
  if ("koller" %in% names(fitted_object)) {
    smdm = unlist(with(fitted_object,
                       list(beta = fixef(koller),
                            variances = c(sigma(koller), 
                                          unlist(lapply(VarCorr(koller), function(z) attr(z, "stddev"))))))
    )
  } else {
    smdm = NULL
  }
  ...
  hat_weighted_rank = unlist(with(fitted_object,
                              list(beta = hat_weighted_rank$beta,
                                   variances = c(hat_weighted_rank$sigma, hat_weighted_rank$theta))
  ))
  
  
  names(lme) <- names(rank) <- names(weighted_rank) <- names(hat_weighted_rank) <-
    c("beta_0", "beta_1", "beta_2", "beta_3", "sigma^2", "theta_0", "theta_1")
...
#' 
replicate_model_fits = function(outlier_size, predictors = "gaussian", with_smdm=FALSE) {
